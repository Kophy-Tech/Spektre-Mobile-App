...
export default function SingleAssignment({navigation}) {
  ...
  const [changeAsignStatus, {isLoading: Loadingstatus}] =
    useChangeAsignStatusMutation();
  const [OpenTicket, {isLoading: LoadingOpenTicket}] = useOpenTicketMutation();
  const [CloseTicket, {isLoading: LoadingCloseTicket}] =
    useCloseTicketMutation();
  ...
  const closeTicketSend = ({idProps, status}) => {
    Alert.alert('Are you sure you want to close this ticket?', '', [
      {
        text: 'Cancel',
        onPress: () => console.log('Cancel Pressed'),
        style: 'cancel',
  ...
  const SendOpenTicket = async () => {
    const d = itemData?.assignees.filter(
      fm => fm.username === responsiblePerson,
    );
  ...
  const renderItemTickets = ({item}) => (
    <Ticket
      item={item}
      closeTicketSend={closeTicketSend}
      LoadingCloseTicket={LoadingCloseTicket}
      IdLoaing={IdLoaing}
  ...
  if (error) {
    if (!error?.status) {
      ...
      return (
        <ErrorCard errormsg="Network request failed, refresh your network and try again!." />
      );
    } else if (error.status === 401) {
      return <ErrorCard errormsg="Unauthorized" />;
    } else {
      return (
        <ErrorCard errormsg="Unknow error occur!,Kindly refresh your application" />
      );
    }
  }
  return (
              ...
              <>
                    ...
                    <Stack p="3" space={3}>
                      ...
                      <HStack
                        ...
                        <Text
                          ...
                          fontWeight="400">
                          {itemData?.project_manager.first_name}{' '}
                          {itemData?.project_manager.last_name}
                        </Text>
                      </HStack>
                      <HStack
                        alignItems="center"
                    ...
                    {itemData?.status}
                      ...
                      <Stack>
                        <Text
                          ...
                          fontWeight="400">
                          Start Time:{' '}
                          {moment(itemData?.start_date).format('Do.MM.YYYY')}
                        </Text>
                      </Stack>
                      <Stack>
                        <Text
                          ...
                          fontWeight="400">
                          Deadline:{' '}
                          {moment(itemData?.deadline).format('Do.MM.YYYY')}
                        </Text>
                      </Stack>
                ...
                <HStack my="2" mx="2" justifyContent="space-between">
                  ...
                  <Button
                    ...
                    <Text color="#4dd3ff" fontSize="xs" fontWeight="800">
                      Dokument hinzufügen
                    </Text>
                  </Button>
                </HStack>
      ...
      <Modal
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <Box justifyContent="flex-end" alignItems="flex-end" my="2">
              <Icon
                name="close"
                size={35}
                color="#4dd3ff"
...
<Icon
            ...
            />
              ...
              <Stack mx="5">
                <TouchableOpacity onPress={selectImage}>
                  <Icon name="image" size={35} color="#4dd3ff" />
                </TouchableOpacity>
              </Stack>
              <Stack mx="5">
                <TouchableOpacity onPress={CameraImage}>
                  <IconI name="camera" size={35} color="#4dd3ff" />
                </TouchableOpacity>
              </Stack>
            </HStack>
      ...
      <Modal
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <HStack justifyContent="space-between" alignItems="flex-end" my="4">
            ...
            <HStack alignItems="center" justifyContent="center">
              <Stack mr="5">
                <TouchableOpacity onPress={documentPicker}>
                  <Icon name="file-upload" size={35} color="#4dd3ff" />
                </TouchableOpacity>
              </Stack>
              <Stack mx="5">
              ...
              <Stack mx="5">
                <TouchableOpacity onPress={CameraImage2}>
                  <IconI name="camera" size={35} color="#4dd3ff" />
                </TouchableOpacity>
              </Stack>
            </HStack>
            ...
            <HStack alignItems="center" my="3" flexWrap="wrap">
              {document.length > 0 &&
                document.map((data, i) => {
                  ...
                  return (
                    <Stack key={i}>
                      {data?.type.includes('jpeg') ||
                      data?.type.includes('png') ? (
                        <Avatar
                          bg="#4dd3ff"
                          mx="1"
                          alignSelf="center"
            ...
            <Box mt="8">
              <Button
                style={{
                  backgroundColor: document.length === 0 ? 'grey' : '#4dd3ff',
                  borderRadius: 10,
                }}
